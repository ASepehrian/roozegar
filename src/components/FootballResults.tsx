...
export default function FootballResults() {
  ...
  return <section className="football-card" aria-label="نتایج لیگ‌های اروپایی">
    ...
    {showTable && <div className="football-standings">
      {tableLoading ? <div className="football-empty">در حال دریافت جدول…</div> : standings?.standings?.length ? <div className="football-table-wrap"><table><thead><tr><th>#</th><th>تیم</th><th>امتیاز</th><th>تفاضل</th><th>بازی</th><th>برد</th><th>باخت</th><th>مساوی</th></tr></thead><tbody>{standings.standings.map((row) => <tr key={`${row.rank}-${row.team}`}><td>{toFa(row.rank)}</td><td><span className="football-table-team"><Logo url={row.logo} />{row.team}</span></td><td><strong>{toFa(row.points)}</strong></td><td>{toFa(row.goalDifference)}</td><td>{toFa(row.played)}</td><td>{toFa(row.wins)}</td><td>{toFa(row.losses)}</td><td>{toFa(row.draws)}</td></tr>)}</tbody></table></div> : <div className="football-empty">جدول این لیگ در دسترس نیست.</div>}
    </div>}
  </section>;
}
